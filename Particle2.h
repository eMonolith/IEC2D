...
class Particle
{
	...
	int x, y;
	int x1, y1, x2, y2;// (x2-x1 * y2-y1)^2
	double xvel, yvel, mass, charge;//-1.60217733E-19;
	public:
	...
	void attract (int pax, int pay); //center x and y
        ...
        void resist (int X, int Y); //particle x and y
...
void Particle::updatePosition ()
{
	if (x != null && walls) //If walls are on and particle is spawned
	{
		...
		if (FrictionOn)
		{
			xvel *= particleSpeed * ATTRACTION_POWER/AIR;  //xvel *= particleSpeed*ATTRACTION_POWER/AIR//xvel *= -1/AIR;    //PV=nRT  P=nRT/V  V=nRT/P  T=PV/AIR AIR=nR?
			yvel *= particleSpeed * ATTRACTION_POWER/AIR;  //yvel *= particleSpeed*ATTRACTION_POWER/AIR//yvel *= -1/AIR    // xvel *= AIR ; yvel *= AIR;
		}
	...
	if (x != null && !walls) //If walls are off
	{
		...
		if (FrictionOn)
		{
			xvel *= particleSpeed * ATTRACTION_POWER/AIR;  //PV=nRT  P=nRT/V  V=nRT/P  T=PV/AIR AIR=nR?
			yvel *= particleSpeed * ATTRACTION_POWER/AIR; // xvel *= AIR ; yvel *= AIR;
//    printf("xvel %lf, yvel %lf\n", xvel, yvel);
		}
		x += round (xvel);  //(v) = sqrt((2eV/m))
		y += round (yvel);
	}
}
...
{
    double r=sqrt(((x2-x1)*(x2-x1))+((y2-y1)*(y2-y1)))/2; //distance between particles
    return r;
}    
...
void Particle::resist (int X, int Y)
{
	if (!isNull ())
	{
		double cAngle = getAngle (x, y, X, Y);
        double r = getDist(x,y,X,Y);
		// e = a*sin(2*pie*f*d) 
		xvel += 2*cosd(cAngle) * (RESIST_POWER/r);
		yvel += 2*sind(cAngle) * (RESIST_POWER/r);
	}
}
