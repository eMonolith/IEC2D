...
void load_files()
{
    ...
    //font = TTF_OpenFont("/usr/share/fonts/truetype/freefont/.ttf", 12);
        font = TTF_OpenFont("/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf", 10);
 ...
 Uint8 *keystate = SDL_GetKeyState(NULL); 
	...
	if (keystate[SDLK_RETURN]) //if (state & SDL_BUTTON(1))
	{
		...
		for (int p = 0; p < 360; p++)		//360 //720 //1000 ? //1080 //1440  //density of emitted circular particle emittions
		{
			...
			double direction = (p+(float)rand()/(float)RAND_MAX);//this works and now I realize that the emitter is square with low values of p
...
// keyboard buttons that are pressed
	...
	if (!keystate[SDLK_SPACE])
	{
		for (int i = 0; i < PARTICLE_COUNT; i++)
		{
			part[i].repell(pax, pay);
			part[i].attract(pax, pay);
		}
	}
    if (keystate[SDLK_y]) {
        ATTRACTION_POWER = ATTRACTION_POWER + 1; }
    if (keystate[SDLK_u]) {
